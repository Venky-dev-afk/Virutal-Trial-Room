...
categories = {
    "formal": {
        "shirts": r"C:\Users\Lenovo\Desktop\Final_Project\app\Formal\Shirts",
        "pants": r"C:\Users\Lenovo\Desktop\Final_Project\app\Formal\Pants"
    },
    "summer": {
        "shirts": r"C:\Users\Lenovo\Desktop\Final_Project\app\Summer\Shirts",
        "pants": r"C:\Users\Lenovo\Desktop\Final_Project\app\Summer\Pants"
    },
    "winter": {
        "shirts": r"C:\Users\Lenovo\Desktop\Final_Project\app\Winter\Shirts",
        "pants": r"C:\Users\Lenovo\Desktop\Final_Project\app\Winter\Pants"
    }
}
...
def generate_frames(category):
    global shirtImageNumber, pantsImageNumber, shirt_offset_x, shirt_offset_y, pants_offset_x, pants_offset_y, listShirts, listPants, shirt_scale, pants_scale
    ...
    while True:
        ...
        if pose_results.pose_landmarks:
            ...
            # Pants overlay
            pants_width = int(abs(lm23_px[0] - lm24_px[0]) * fixedRatio * pants_scale)
            ...
            # Shirt overlay
            shirt_width = int(abs(lm11_px[0] - lm12_px[0]) * fixedRatio* shirt_scale)
            ...
            with mic as source:
                ...
                if len(listShirts) > 0:
                    if "next shirt" in command:
                        ...
                        Thread(target=speak_text, args=("Next shirt selected",)).start()

                    elif "previous shirt" in command:
                        shirtImageNumber = (shirtImageNumber - 1) % len(listShirts)
                        Thread(target=speak_text, args=("Previous shirt selected",)).start()

                if len(listPants) > 0:
                    if "next pants" in command:
                        pantsImageNumber = (pantsImageNumber + 1) % len(listPants)
                        Thread(target=speak_text, args=("Next pants selected",)).start()

                    elif "previous pants" in command:
                        pantsImageNumber = (pantsImageNumber - 1) % len(listPants)
                        Thread(target=speak_text, args=("Previous pants selected",)).start()

                # Offset adjustment commands remain the same
                if "shirt up" in command:
                    shirt_offset_y -= 5
                    Thread(target=speak_text, args=("Shirt moved up",)).start()

                elif "shirt down" in command:
                    shirt_offset_y += 5
                    Thread(target=speak_text, args=("Shirt moved down",)).start()

                elif "pants up" in command:
                    pants_offset_y -= 5
                    Thread(target=speak_text, args=("Pants moved up",)).start()

                elif "pants down" in command:
                    pants_offset_y += 5
                    Thread(target=speak_text, args=("Pants moved down",)).start()
...
def update_offsets():
    ...
    global shirtImageNumber, pantsImageNumber, shirt_offset_x, shirt_offset_y, pants_offset_x, pants_offset_y,shirt_scale, pants_scale
    ...
    if key == 'w':  # Move shirt up
        ...
        shirtImageNumber = (shirtImageNumber + 1) % len(listShirts)
    elif key == 'p':  # Previous shirt image
        shirtImageNumber = (shirtImageNumber - 1) % len(listShirts)
    elif key == 'm':  # Next pants image
        pantsImageNumber = (pantsImageNumber + 1) % len(listPants)
    elif key == 'b':  # Previous pants image
        pantsImageNumber = (pantsImageNumber - 1) % len(listPants)
    elif key == 'z':  # Increase shirt size
        shirt_scale += 0.1
    elif key == 'x':  # Decrease shirt size
        shirt_scale = max(0.1, shirt_scale - 0.1)
    elif key == 'c':  # Increase pants size
        pants_scale += 0.1
    elif key == 'v':  # Decrease pants size
        pants_scale = max(0.1, pants_scale - 0.1)
